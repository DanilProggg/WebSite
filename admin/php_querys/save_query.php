...
$result = mysqli_query($db, sprintf("SELECT `id_расписания` FROM `расписание` WHERE `дата`='%s' AND`группа`='%s'",
...
if($replace_row == null){
	...
	mysqli_query($db, sprintf("INSERT INTO `расписание`(`дата`, `группа`, `дисциплины`) VALUES ('%s','%s','%s')",
		mysqli_real_escape_string($db,$date),
		mysqli_real_escape_string($db,$group),
